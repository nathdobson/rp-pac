...
impl Ctrl {
    ...
    #[doc = "When 1, writes to any alias other than 0x0 (caching, allocating) will produce a bus fault. When 0, these writes are silently ignored. In either case, writes to the 0x0 alias will deallocate on tag match, as usual."]
    #[inline(always)]
    pub const fn set_err_badwrite(&mut self, val: bool) {
...
impl defmt::Format for Ctrl {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Ctrl {{ en: {=bool:?}, err_badwrite: {=bool:?}, power_down: {=bool:?} }}",
            self.en(),
            self.err_badwrite(),
            self.power_down()
        )
...
impl defmt::Format for Flush {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(f, "Flush {{ flush: {=bool:?} }}", self.flush())
    }
}
#[doc = "Cache Status."]
#[repr(transparent)]
...
impl Stat {
    ...
    #[doc = "Reads as 0 while a cache flush is in progress, and 1 otherwise. The cache is flushed whenever the XIP block is reset, and also when requested via the FLUSH register."]
    #[inline(always)]
    pub const fn set_flush_ready(&mut self, val: bool) {
        self.0 = (self.0 & !(0x01 << 0usize)) | (((val as u32) & 0x01) << 0usize);
    }
...
impl defmt::Format for Stat {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Stat {{ flush_ready: {=bool:?}, fifo_empty: {=bool:?}, fifo_full: {=bool:?} }}",
            self.flush_ready(),
            self.fifo_empty(),
            self.fifo_full()
        )
