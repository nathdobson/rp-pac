...
impl defmt::Format for Ctrl {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "Ctrl {{ sm_enable: {=u8:?}, sm_restart: {=u8:?}, clkdiv_restart: {=u8:?} }}",
            self.sm_enable(),
            self.sm_restart(),
            self.clkdiv_restart()
        )
...
impl defmt::Format for DbgCfginfo {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "DbgCfginfo {{ fifo_depth: {=u8:?}, sm_count: {=u8:?}, imem_size: {=u8:?} }}",
            self.fifo_depth(),
            self.sm_count(),
            self.imem_size()
        )
...
impl defmt::Format for Intr {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "Intr {{ sm0_rxnempty: {=bool:?}, sm1_rxnempty: {=bool:?}, sm2_rxnempty: {=bool:?}, sm3_rxnempty: {=bool:?}, sm0_txnfull: {=bool:?}, sm1_txnfull: {=bool:?}, sm2_txnfull: {=bool:?}, sm3_txnfull: {=bool:?}, sm0: {=bool:?}, sm1: {=bool:?}, sm2: {=bool:?}, sm3: {=bool:?} }}" , self . sm0_rxnempty () , self . sm1_rxnempty () , self . sm2_rxnempty () , self . sm3_rxnempty () , self . sm0_txnfull () , self . sm1_txnfull () , self . sm2_txnfull () , self . sm3_txnfull () , self . sm0 () , self . sm1 () , self . sm2 () , self . sm3 ())
    }
}
...
impl SmExecctrl {
    ...
    #[doc = "Comparison used for the MOV x, STATUS instruction."]
    #[inline(always)]
    pub const fn set_status_sel(&mut self, val: super::vals::SmExecctrlStatusSel) {
...
impl defmt::Format for SmShiftctrl {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "SmShiftctrl {{ autopush: {=bool:?}, autopull: {=bool:?}, in_shiftdir: {=bool:?}, out_shiftdir: {=bool:?}, push_thresh: {=u8:?}, pull_thresh: {=u8:?}, fjoin_tx: {=bool:?}, fjoin_rx: {=bool:?} }}" , self . autopush () , self . autopull () , self . in_shiftdir () , self . out_shiftdir () , self . push_thresh () , self . pull_thresh () , self . fjoin_tx () , self . fjoin_rx ())
    }
}
