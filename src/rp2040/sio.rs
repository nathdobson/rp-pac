...
impl Sio {
    ...
    pub const fn gpio_out(self, n: usize) -> Gpio {
        assert!(n < 2usize);
        unsafe { Gpio::from_ptr(self.ptr.wrapping_add(0x10usize + n * 32usize) as _) }
    }
    #[inline(always)]
    pub const fn gpio_oe(self, n: usize) -> Gpio {
        assert!(n < 2usize);
        unsafe { Gpio::from_ptr(self.ptr.wrapping_add(0x20usize + n * 32usize) as _) }
    }
    #[inline(always)]
    pub const fn fifo(self) -> Fifo {
        unsafe { Fifo::from_ptr(self.ptr.wrapping_add(0x50usize) as _) }
    }
    ...
    pub const fn div(self) -> Div {
        unsafe { Div::from_ptr(self.ptr.wrapping_add(0x60usize) as _) }
    }
    #[inline(always)]
    pub const fn interp(self, n: usize) -> Interp {
        assert!(n < 2usize);
        unsafe { Interp::from_ptr(self.ptr.wrapping_add(0x80usize + n * 64usize) as _) }
    }
