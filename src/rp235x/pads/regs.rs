...
impl defmt::Format for GpioCtrl {
    fn format(&self, f: defmt::Formatter) {
        defmt :: write ! (f , "GpioCtrl {{ slewfast: {=bool:?}, schmitt: {=bool:?}, pde: {=bool:?}, pue: {=bool:?}, drive: {:?}, ie: {=bool:?}, od: {=bool:?}, iso: {=bool:?} }}" , self . slewfast () , self . schmitt () , self . pde () , self . pue () , self . drive () , self . ie () , self . od () , self . iso ())
    }
}
