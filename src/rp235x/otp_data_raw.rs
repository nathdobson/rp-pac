...
impl OtpDataRaw {
    ...
    pub const fn bootkey(self, n: usize) -> BootKey {
        assert!(n < 4usize);
        unsafe { BootKey::from_ptr(self.ptr.wrapping_add(0x0200usize + n * 64usize) as _) }
    }
    #[inline(always)]
    pub const fn key(self, n: usize) -> Key {
        assert!(n < 6usize);
        unsafe { Key::from_ptr(self.ptr.wrapping_add(0x3d20usize + n * 32usize) as _) }
    }
    ...
    pub const fn key_valid(
        ...
        assert!(n < 6usize);
        unsafe {
            crate::common::Reg::from_ptr(self.ptr.wrapping_add(0x3de4usize + n * 4usize) as _)
        }
    }
    #[inline(always)]
    pub const fn page_lock(self, n: usize) -> PageLock {
        assert!(n < 64usize);
        unsafe { PageLock::from_ptr(self.ptr.wrapping_add(0x3e00usize + n * 8usize) as _) }
    }
}
#[derive(Copy, Clone, Eq, PartialEq)]
pub struct PageLock {
