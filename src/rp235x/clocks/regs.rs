...
#[repr(transparent)]
#[derive(Copy, Clone, Eq, PartialEq)]
pub struct ClkGpoutSelected(pub u32);
impl ClkGpoutSelected {
    #[doc = "This slice does not have a glitchless mux (only the AUX_SRC field is present, not SRC) so this register is hardwired to 0x1."]
    #[must_use]
    #[inline(always)]
...
impl Default for ClkGpoutSelected {
    #[inline(always)]
    fn default() -> ClkGpoutSelected {
        ClkGpoutSelected(0)
    }
}
impl core::fmt::Debug for ClkGpoutSelected {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        f.debug_struct("ClkGpoutSelected")
            .field("clk_gpout0_selected", &self.clk_gpout0_selected())
            .finish()
    }
}
#[cfg(feature = "defmt")]
impl defmt::Format for ClkGpoutSelected {
    fn format(&self, f: defmt::Formatter) {
        defmt::write!(
            f,
            "ClkGpoutSelected {{ clk_gpout0_selected: {=bool:?} }}",
