...
impl Pio {
    ...
    pub const fn sm(self, n: usize) -> StateMachine {
        assert!(n < 4usize);
        unsafe { StateMachine::from_ptr(self.ptr.wrapping_add(0xc8usize + n * 24usize) as _) }
    }
    #[inline(always)]
    pub const fn rxf_putget(self, n: usize) -> RxfPutGet {
        assert!(n < 4usize);
        unsafe { RxfPutGet::from_ptr(self.ptr.wrapping_add(0x0128usize + n * 16usize) as _) }
    }
    ...
    pub const fn irqs(self, n: usize) -> Irq {
        assert!(n < 2usize);
        unsafe { Irq::from_ptr(self.ptr.wrapping_add(0x0170usize + n * 12usize) as _) }
    }
}
#[derive(Copy, Clone, Eq, PartialEq)]
